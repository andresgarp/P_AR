...
% If we have
%   Xv(k) = Xv(k-1) + [v*dt*cos(g+XX(3)); v*dt*sin(g+XX(3));%   V*dt*sin(g)/WB)]
%   U = [v; g]

% calcular e incluir los jacobianos de prediccion (vehiculo)
% jacobians   
Gv = [1 0 -vts; 0 1 vtc; 0 0 1]; % dXv(k)/dXv(k-1)
Gu = [dt*c -vts; dt*s vtc; dt*sin(g)/WB v*dt*cos(g)/WB]; % dXv(k)/dU
